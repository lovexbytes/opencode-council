...
export async function loadCouncilConfig(projectDir) {
    ...
    if (!raw) {
        throw new Error(`Council configuration not found. Create ${DEFAULT_CONFIG_BASENAME} in ${path.join(projectDir, ".opencode")} or ~/.config/opencode/ (or set OPENCODE_COUNCIL_CONFIG).`);
    }
    let parsed;
    try {
